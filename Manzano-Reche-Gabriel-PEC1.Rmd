...
Para esta PEC1 se ha escogido el dataset propuesto llamado **2018-Phosphoproteomics**. 
...
-   Los modelos PDX pueden presentan variabilidad biológica, lo que puede influir en los resultados.
...
A continuación se definen los metadatos de los fosfopéptidos, una descripción de estos y la modificacion en su secuencia.
...
boxplot(log10(assay(se, "counts") + 1), 
        ...
        las = 2,  # Gira las etiquetas del eje x para facilitar la lectura
...
Para ello, en primer ligar se crea la matriz de diseño, en la que a cada muestra se le aplica un 1 y un 0, dependiendo de si está presente o no en los grupos definidos.
...
results_df <- data.frame(
  logFC = fit2$coef[, "mainEff"],           # Cambio de expresión log2 fold change
  P.Value = fit2$p.value[, "mainEff"],      # Valor p
  adj.P.Val = p.adjust(fit2$p.value[, "mainEff"], method = "BH")  # Valor p ajustado
)
...
Como se puede apreciar en el VolcanoPlot no se observan diferencias significativas entre grupos, pero esto puede deberse a a alta variabilidad, tamaño pequeño de muestra y a las similitudes que presentan ambos grupos. Por ello, a continuación, se van a aplicar técnicas de normalización y escalado para reducir la dispersión y ser capaz de detectar diferencias relevantes.

En primer lugar, se va a aplicar la función *normalizeQuantiles* que ajusta las distribuciones en todas las muestras para que sean i´denticas, posteriormente se aplicará *log2* para reducir la varianza y finalmente, se escalarán los datos para que tengan media cero y desviación estándar uno.


```{r}
# Paso 1: Aplicar normalización de cuantiles
abundancias_norm <- normalizeQuantiles(abundancias)

# Paso 2: Aplicar transformación logarítmica para estabilizar la varianza
abundancias_norm_log <- log2(abundancias_norm + 1) 

# Paso 3: Escalado Z-score (escalado por fila) para cada fosfopéptido
abundancias_final <- t(apply(abundancias_norm_log, 1, scale))

# Definir los grupos y el diseño del modelo
targets <- as.data.frame(targets)
groups <- as.factor(targets$Phenotype)
designMat <- model.matrix(~ -1 + groups)

# Calcular la correlación entre réplicas técnicas
dupcor <- duplicateCorrelation(abundancias_final, designMat, block = targets$Individual)

# Crear la matriz de contraste para comparar PD y MSS
contMat <- makeContrasts(mainEff = groupsPD - groupsMSS, levels = designMat)

# Ajuste del modelo con los datos normalizados y transformados
fit <- lmFit(abundancias_final, designMat, block = targets$Individual, correlation = dupcor$consensus)
fit2 <- contrasts.fit(fit, contMat)
fit2 <- eBayes(fit2)

# Extraer los resultados en formato de dataframe para ggplot2
results_df <- data.frame(
...
# Añadir una columna de significancia para el Volcano Plot
results_df <- results_df %>%
  mutate(significant = ifelse(adj.P.Val < 0.05 & abs(logFC) > 1, "Significant", "Not Significant"))

# Crear y visualizar el Volcano Plot con ggplot2
ggplot(results_df, aes(x = logFC, y = -log10(P.Value), color = significant)) +
  geom_point(alpha = 0.8) +
  scale_color_manual(values = c("Significant" = "red", "Not Significant" = "gray")) +
  labs(title = "Volcano Plot de Expresión Diferencial",
...
Como se puede apreciar en el gráfico, se ha reducido la variabilidad considerablente, sin embargo, siguen sin detectarse diferencias significativas entre los grupos. Esto puede deberse a la similitud biológica en términos de fosfopéptidos de ambos grupos.
